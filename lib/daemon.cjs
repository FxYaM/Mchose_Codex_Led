...
async function runDaemon(packageDirectory, explicitConfigPath) {
  ...
  async function restoreLoadedSnapshot(loaded, acceptDevice, reason) {
  ...
  async function handleRequestedRestore() {
    ...
    await restoreLoadedSnapshot(loaded, request.acceptDevice === true, "explicit restore request");
  ...
  async function recoverPendingIfNeeded() {
    ...
    await restoreLoadedSnapshot(loaded, false, "unclean-exit or active takeover recovery");
    return true;
  }
  ...
  async function reconcileOnce() {
    ...
    if (shuttingDown) {
      ...
      if (pending) {
        ...
        await restoreLoadedSnapshot(loaded, false, "daemon shutdown");
        deviceObserved = true;
      }
    ...
    if (preset.mode === "preserve") {
      ...
      if (pending) {
        const loaded = currentOwnership || loadSnapshot(pending.snapshotDirectory);
        await restoreLoadedSnapshot(loaded, false, `${desired} state releases controller ownership`);
        deviceObserved = true;
      } else {
    ...
    switch (message.type) {
      ...
      case "restore":
        requestedRestore = {
          ...
          acceptDevice: message.acceptDevice === true,
        };
        requestReconcile();
