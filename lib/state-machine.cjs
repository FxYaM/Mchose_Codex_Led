...
class TaskStateMachine {
  ...
  recordOutcome(outcome) {
    if (!outcome) return;
    if (!this.batch) this.batch = { id: crypto.randomUUID(), startedAt: Date.now(), outcome: null };
  ...
  showUpgradedTerminalOutcome(outcome, now) {
    if (!outcome) return;
    if (this.tasks.size > 0) {
      this.recordOutcome(outcome);
      return;
    }
    if (!this.batch) this.batch = { id: crypto.randomUUID(), startedAt: now, outcome: null };
    this.recordOutcome(outcome);
    this.finalizeBatch(now);
  }
    ...
    if (terminalKinds.has(event.kind)) {
      ...
      if (previousTerminal) {
        ...
        if (terminalSeverity(outcome) > terminalSeverity(previousTerminal.outcome)) {
          ...
          this.showUpgradedTerminalOutcome(outcome, now);
        }
