...
The 1.6.1 local checks on Windows and Node.js 24.19.0 passed **89/89 tests**, checks for 25 CJS files and six PowerShell scripts, and a fresh dependency installation with native-module loading. The Windows regression suite installs using a real 8.3 profile path and uninstalls using its long-path equivalent, verifying both startup and hook removal. Hardware behavior was not revalidated for this release: physical unplug/replug, reboot startup, real keyboard readback and live Desktop integration remain separate manual checks. Earlier successful observations do not count as new release verification.
